...
case "$STEP" in
  ...
  1)
    ...
    check "Score: first to 5"                        "scoreToWin.*5\|5.*scoreToWin\|toWin.*5\|winScore.*5\|WIN_SCORE.*5\|score.*>=.*5\|>= 5\|=== 5\|>= WIN_SCORE"
  ...
  2)
    ...
    check "Hidden input for mobile keyboard"          "nameInput\|opacity:0\|opacity: 0\|pointer-events:none\|pointerEvents.*none\|#nameInput"
    check "Setup screen"                              "setupScreen\|PHASE_SETUP\|phase.*setup\|screen.*setup\|setup.*screen\|SETUP_ROWS\|Easy.*Medium.*Hard"
    check "Difficulty: Easy/Medium/Hard"              "Easy.*Medium.*Hard\|Easy.*speed\|Medium.*speed\|label.*Easy\|id.*easy"
    ...
    check "P2 I/K keyboard controls"                  "KeyI\|KeyK\|['\"]i['\"].*['\"]k['\"]\\|['\"]I['\"].*['\"]K['\"]\\|key.*[iI].*[kK]\\|I.*K.*keys"
    ...
    check "Click sound for buttons"                   "sndClick\|sfxClick\|click.*sound\|clickSnd\|sfx.*click\|Click()"
